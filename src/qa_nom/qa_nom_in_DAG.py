...
import src.DAG.NounPhraseObject as NounPhrase
...
import sys
from src.question_to_query import utils as question_to_query_utils
from src.topic_clustering import utils_clustering as utils_clustering
import spacy
import torch
...
def get_predicate_to_question_and_question_to_nodes_nested_dictionary(span_to_qanom_object_lst, dict_span_to_object,
    ...
    for span, qanom_object_lst in span_to_qanom_object_lst.items():
        ...
        for qanom_object in qanom_object_lst:
            predicate = ut.dict_word_to_lemma.get(qanom_object.predicate, qanom_object.predicate)
...
def get_potential_qa_nom(topic_objects):
    ...
    for span, qanom_object_lst in span_to_qanom_object_lst.items():
        for qanom_object in qanom_object_lst:
            predicate_lst.add(ut.dict_word_to_lemma.get(qanom_object.predicate, qanom_object.predicate))
...
def initialize_data():
    topic_objects = pickle.load(open("../../results/results_disease/diabetes/topic_object_lst.p", "rb"))
    global_index_to_similar_longest_np = pickle.load(
        open("../../results/results_disease/diabetes/global_index_to_similar_longest_np.p", "rb"))
    dict_span_to_rank = pickle.load(open("../../results/results_disease/diabetes/dict_span_to_rank.p", "rb"))
    global_dict_label_to_object = pickle.load(open(
        "../../results/results_disease/diabetes/global_dict_label_to_object.p", "rb"))
    visited = set()
    dict_span_to_object = {}
